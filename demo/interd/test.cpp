...
void plotinterd() {
    ...
    for(int i=0; i<n-1; i++) {
        double vmin = *std::min_element(var[2*(i*(n-1)+i)+1].begin(), var[2*(i*(n-1)+i)+1].end());
        double vmax = *std::max_element(var[2*(i*(n-1)+i)+1].begin(), var[2*(i*(n-1)+i)+1].end());
        if(vmin < v_min) {
            v_min = vmin;
        }
        if(vmax > v_max) {
    ...
    for(int i=0; i<n-1; i++) {
        ...
        for(int nx=0; nx<N; nx++) {
            data[var[2*(i*(n-1)+i)][nx]] = var[2*(i*(n-1)+i) + 1][nx];
        }
        std::string label = std::string("\\tilde D_") + std::to_string(i) + std::to_string(i) + "^" + std::to_string(n-1);
